...
export default function FifaKnockoutView() {
  ...
  useEffect(() => {
    if (hasAdminBracket && fifaKnockout.length === 0) {
      useQuinielaStore.getState().initFifaKnockout()
    }
  ...
  if (!hasAdminBracket && fifaKnockout.length === 0) {
    return (
      <div className="bg-gray-800/60 backdrop-blur rounded-xl border border-gray-700/50 p-8 text-center">
        ...
        <h3 className="text-lg font-semibold text-white mb-2">Fase Final FIFA</h3>
        <p className="text-gray-400 text-sm max-w-md mx-auto">
          El administrador aún no ha cargado los brackets oficiales de la FIFA.
          Vuelve más tarde para capturar tus marcadores.
        </p>
      </div>
    )
  }
  ...
  const getMatches = (round: string) =>
    getMatchesByRound(mergedKnockout, round)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-white">Fase Final FIFA</h2>
